...
class Table {
    ...
    renderHand(hand, playerName) {
        ...
        handHolder.classList.add("hand", playerName);// this.players);
        console.log(hand.length, playerName)
        for (let i = 0; i < hand.length; i++) {
            ...
            button.classList.add(cardSuit + cardFace, playerName)
...
dealButton.addEventListener("click", () => {
    ...
    //total = gameBoard.calcTotal(playerHand)
    console.log(gameBoard)
    console.log(gameBoard.players[0].playerCards, gameBoard.players[0].playerName);
    console.log(gameBoard.players[1].playerCards, gameBoard.players[1].playerName);
    console.log(gameBoard.deck.cards)
    console.log('calc total: ',gameBoard.calcTotal(playerHand))
    let newCardBtns = document.querySelectorAll('.newCard')
    console.log(newCardBtns)
    newCardBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            let player = btn.classList[2].toLowerCase();
            console.log(btn.classList[1])
            console.log(player)
            console.log(gameBoard.deck)
            gameBoard.getNewCard(player + 'hand')
        })
